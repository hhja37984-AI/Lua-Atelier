...
function allVars(arr,into=new Set(['input'])){for(const b of arr){if(b.name)into.add(b.name);if(b.type==='split')b.names.split(',').map(x=>x.trim()).filter(Boolean).forEach(x=>into.add(x));if(b.type==='pattern')b.parts.map(x=>x.name).filter(Boolean).forEach(x=>into.add(x));if(b.type==='if'){allVars(b.yes,into);allVars(b.no,into)}}return into}
function redraw(){
