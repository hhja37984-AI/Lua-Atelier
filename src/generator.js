...
function buildLua(){const isEdit=project.trigger.startsWith('edit');const parts=['-- Risu Lua Atelier v0.4',''];let flows=[project.flow,...project.functions.map(f=>f.flow)];const hasType=t=>flows.some(f=>JSON.stringify(f).includes('"type":"'+t+'"'));if(['split','extract','replace','weighted','date'].some(hasType))parts.push(HELPERS,'');
...
parts.push(...codeFlow(project.flow,new Set(['input','data']),1,'main'));if(isEdit)parts.push('    return data');parts.push('end'+(isEdit?')':''));return parts.join('\n');}
function diagnose(){let errors=[];let inspect=(arr,ctx,known)=>{for(const b of arr){let names=[];if(b.name)names.push(b.name);if(b.type==='split')names.push(...b.names.split(',').map(x=>x.trim()).filter(Boolean));if(b.type==='pattern')names.push(...b.parts.map(x=>x.name).filter(Boolean));for(const n of names)if(!validName(n))errors.push('변수 이름 확인: '+n);names.forEach(x=>known.add(x));if(b.type==='stateSet'&&!project.stateKey)errors.push('State key가 비어 있습니다.');if(b.type==='call'){let f=project.functions.find(x=>x.id===b.fn);if(!f)errors.push('내 함수 호출: 함수를 선택하세요.');else if(b.args.split(',').filter(x=>x.trim()).length!==f.params.split(',').filter(x=>x.trim()).length)errors.push('함수 '+f.name+': 입력 인자 수를 확인하세요.');}if(b.type==='output'&&!project.trigger.startsWith('edit'))errors.push('텍스트 출력은 edit 트리거에서 사용하세요.');if(b.type==='if'){inspect(b.yes,ctx,new Set(known));inspect(b.no,ctx,new Set(known))}if(b.type==='return'&&ctx==='main')errors.push('함수 반환 블록은 내 함수 안에 배치하세요.');if(b.type==='pattern'&&b.parts.length===0)errors.push('패턴 추출: 문자 규칙을 추가하세요.');}};
